...
exports.updateVehicle = async (req, res) => {
  try {
    const vehicleId = req.params.id;
    const { make, model, year, pricePerDay, description, availability } =
      req.body;
...
exports.deleteReservation = async (req, res) => {
  try {
    ...
    if (end < today) {
      // If the end date is in the past, update the reservation status to 'Cancelled'
      await Reservation.findByIdAndUpdate(req.params.id, {
        status: "Cancelled",
      });
      ...
      if (user) {
        await sendCancellationNotification(user.email, reservation);
      }
    } else {
      // If the reservation is still valid, proceed with deletion
...
// Function to send a cancellation notification (same as before)
const sendCancellationNotification = async (email, reservation) => {
  ...
  let mailOptions = {
    ...
    to: email,
    subject: `Reservation of ${reservation.vehicle.make} ${reservation.vehicle.model} Cancelled`,
    text: `Dear User,

Your reservation for ${reservation.vehicle.make} ${reservation.vehicle.model} has been cancelled as the end date has passed.
